...
public class StealthKill : MonoBehaviour
{
    ...
    //New code for stealth animation
    [Header("New Stealth")]
    [SerializeField] private float distance;
    [SerializeField] Vector3 dir;
    public Transform enemyREF;
    public float offset,radius;
    public Transform EnemyRef;
    int mask = 11;
    RaycastHit hit1;
    public bool stealth;
    public void TakeDown(InputAction.CallbackContext callbackContext)
    {
        stealth = Physics.SphereCast(transform.position, radius, transform.position + dir, out hit1, offset,LayerMask.GetMask("Enemy"));
        if (hit1.collider != null) { Debug.Log(hit1.collider.gameObject.name); }
    ...
    private void Update()
    {
        if(Physics.SphereCast(transform.position, radius, transform.position + dir, out hit1, offset, LayerMask.GetMask("Enemy")))
    ...
    private void OnDrawGizmos()
    {
        ...
        Vector3 endPosition = startPosition + direction * offset;
        ...
        // Draw the line representing the path
